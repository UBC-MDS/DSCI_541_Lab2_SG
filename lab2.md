...
1. Some ways that the way terms of conditions can be improved from what they are right now is to have an abstract or bullet points on the top so that the reader would be able to summarize the terms of conditions to an acceptable level. Doing this will allow a larger amount of individuals to know what they are signing up for instead of having to navigate through a complicated write up. We also discussed that we believe that organizations purposely make terms of conditions more complicated to read so they can get away with fine print; users will be accepting certain things without even knowing those conditions were there as they couldn't comprehend the writing. To avoid this, there should be some type of government regulation so that terms of conditions written in a way that anyone can read them. A suggestion we came up with was to have a FAQ section for the terms of conditions or even a chatbot where users can ask certain questions rather than having to search through all the heavy text.
